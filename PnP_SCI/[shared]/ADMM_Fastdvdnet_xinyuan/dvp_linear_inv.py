...
def gap_denoise_bayer_demosaic(y_bayer, Phi_bayer, _lambda=1, accelerate=True, 
                denoiser='tv', iter_max=50, noise_estimate=True, sigma=None, 
                tv_weight=0.1, tv_iter_max=5, multichannel=True, x0_bayer=None, 
                X_orig=None, model=None, show_iqa=True,demosaic_method = 'malvar2004',logf = None):
    '''
            ...
            #     x = np.transpose(x.squeeze(3),(1,2,0))
                ...
                for imask in range(nmask):
                    if demosaic_method.lower == 'bilinear':
                        ...
                        x_rgb[:,:,:,imask] = demosaicing_CFA_Bayer_Malvar2004(x_bayer[:,:,imask])
                    else:
                ...
                for imask in range(nmask):
                    if demosaic_method.lower == 'bilinear':
                        ...
                        x_rgb[:,:,imask,:] = demosaicing_CFA_Bayer_Menon2007(x_bayer[:,:,imask])
                xrgb1 = fastdvdnet_denoiser(x_rgb, nsig, model)
                #xrgb1 = np.single(xrgb1)//255;
                xall[...,0] = xrgb1[0::2,0::2,:,0] # R  channel (average over two)
                xall[...,1] = xrgb1[0::2,1::2,:,1] # G1=G2 channel (average over two)
                xall[...,2] = xrgb1[1::2,0::2,:,1] # G2=G1 channel (average over two)
                xall[...,3] = xrgb1[1::2,1::2,:,2] # B  channel (average over two)
            else:
                ...
                if (k+1)%2 == 0:
                    if not noise_estimate and nsig is not None:
                        if nsig < 1:
                            ...
                            logf.write('  GAP-{0} iteration {1: 3d}, sigma {2: 3g}/255, ' 
                            ...
                            print('  GAP-{0} iteration {1: 3d}, sigma {2: 3g}, ' 
                                'PSNR {3:2.2f} dB.'.format(denoiser.upper(), 
                                k+1, nsig, psnr_all[k]))
                            logf.write('  GAP-{0} iteration {1: 3d}, sigma {2: 3g}, ' 
                                'PSNR {3:2.2f} dB.\n'.format(denoiser.upper(), 
                                k+1, nsig, psnr_all[k]))
                    else:
                        print('  GAP-{0} iteration {1: 3d}, ' 
                            ...
                            k+1, psnr_all[k]))
                        logf.write('  GAP-{0} iteration {1: 3d}, ' 
                            'PSNR {2:2.2f} dB.\n'.format(denoiser.upper(), 
                            k+1, psnr_all[k]))
            k = k+1
    ...
    for idx, nsig in enumerate(sigma): # iterate all noise levels
        for it in range(iter_max[idx]): 
            ...
            #     x = np.transpose(x.squeeze(3),(1,2,0))
                ...
                #    x_rgb[:,:,nt,:] = cv2.cvtColor(np.uint8(x_bayer[:,:,nt]*255), cv2.COLOR_BAYER_RG2BGR)
                    x_rgb[:,:,imask,:] = demosaicing_CFA_Bayer_Menon2007(x_bayer[:,:,imask])
                # # option 1 - run denoising twice
                # xrgb1 = xall[..., [0,1,3]] # R-G1-B (H x W x F x C)
                # xrgb2 = xall[..., [0,2,3]] # R-G2-B (H x W x F x C)
...
def gap_denoise_gray(y, Phi_sum, A, At, _lambda=1, accelerate=True, 
