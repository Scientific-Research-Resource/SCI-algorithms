function  [im, data_save]   =   TV_res_GAP_CACTI( y, nr,para, M_func,Mt_func)
     ...
     if (mod(iter, 10) == 1)
        if isfield(para,'ori_im')
            %PSNR     =   psnr( f./max(f(:)), para.ori_im./max(para.ori_im(:))); 
             PSNR     =   psnr( f, para.ori_im, max(para.ori_im(:))); 
            fprintf( 'TV-res-clip2D-GAP Compressive Image Recovery, Iter %d : PSNR = %f\n', iter, PSNR );
        end
     end
     for ii = 1 : 1
            fb        =   A( f );
        ...
        %for nn = 1:nc 
         f_res         =   TV_denoising(f-x_ave,  para.TVweight,4);  
         f = f_res + x_ave;
