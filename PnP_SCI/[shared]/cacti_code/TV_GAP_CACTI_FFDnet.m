function  [im, PSNR_save]   =   TV_GAP_CACTI_FFDnet( y, para, M_func,Mt_func)
...
for isig = 1:(length(para.sigma)+1)
   ...
   %  for ii = 1 : 1
         fb        =   A( f );
            ...
            for ncc = 1:nc
            f(:,:,ncc)         =  FFD_Net_Denoiser2(squeeze(f(:,:,ncc)), sig_use,net); %(f,  para.TVweight,5);  TV_GAP_CACTI_FFDnet
            end
            %f = f./255;
