function  [im, data_save]   =   TV_GAP_CACTI( y, nr,para, M_func,Mt_func)
...
for  iter = 1 : para.iter   
     if (mod(iter, 10) == 1)
        if isfield(para,'ori_im')
            ...
            fprintf( 'TVclip2D-GAP Compressive Image Recovery, Iter %d : PSNR = %f\n', iter, PSNR );
        end
     end
     for ii = 1 : 1
            fb        =   A( f );
        ...
        %for nn = 1:nc 
         f         =   TV_denoising(f,  para.TVweight,4);  
