...
class VideoPatchSearch():
    ...
    def __init__(self, step=1, patch_search_width=7, patch_data_width=7, past_frames=4, future_frames=4, search_width=21, input_dtype=np.float32):
        ...
        ctx = cl.create_some_context() # implicit context requiring manual input export PYOPENCL_CTX = '0:0'
        # platform = cl.get_platforms()[0]    # Select the first platform [0]
        # device = platform.get_devices()[0]  # Select the first device on this platform [0]
        # ctx = cl.Context([device])          # Create a context with your device

        queue = cl.CommandQueue(ctx)
        cl_file = 'video_patch_search.cl'
        cur_dir = os.path.dirname(__file__)
        f = open(os.path.join(cur_dir, cl_file), 'r')
