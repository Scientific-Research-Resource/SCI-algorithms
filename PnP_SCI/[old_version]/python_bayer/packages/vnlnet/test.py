...
from .psnr import *# image quality metrics (Peak-Signal-to-Noise Ratio, PSNR)
from .ssim import * # image quality metrics (Structural SIMilarity, SSIM)
...
def load_net(net_path, cuda, net_dir='./pretrained/'):
    if not(os.path.isfile(net_path)):
        net_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), net_dir, net_path)
        if not(os.path.isfile(net_path)):
            print(net_path)
            assert(False)
    if cuda:
...
def test(training_args, model_state_dict, video, foutnames, only_frame, cuda, add_noise):
    ...
    # print ("Training args:") # optional print
    # print (training_args) # optional print
    ...
    for i in range(video.shape[0]):
        ...
        if training_args.pass_nn_value: # include non-local features
            start_time = time.time() 
            # search non-local video patches via (Py)OpenCL
            nearest_neighbors_indices = ps.compute(video_search_extract_gray, training_args.past_frames)
            img_noised_patch_stack = ps.build_neighbors_array(video_extract, nearest_neighbors_indices[:-(2*border_size),:-(2*border_size),:])
            # print('  Frame #{:03d} non-local search takes {:3.3f} seconds.'.format(i+1, time.time() - start_time))
            nntime.append(time.time() - start_time)
        else: # simplified DnCNN (no patch)
        ...
        with torch.no_grad():
            ...
            # print(img_noised_patch_stack.shape)
            ...
            video_denoised.append(outimg) # append denoised frame
            # write_file(foutnames[i], outimg) # write denoised images to file
            ...
            """
    # print('Total non-local search takes {:3.3f} seconds.'.format(sum(nntime)))
    return np.ascontiguousarray(video_denoised), sum(nntime)

def vnlnet(video, sigma):
    """
    VNLnet for video denoising.

    Parameters
    ----------
    video : array_like
        The video of 3-D (mono) or 4-D (color) to be denoised.
    sigma : int or float
        The standard deviation of the noise in the video, whether exact or estimated.

    Returns
    -------
    vdenoised : array_like
        The denoised video by VNLnet.
    """
    cpu = False # run model on GPU (not CPU)
    # print(video.shape)
    if video.shape[3]>1: # color video
        net = 'vnlnet_color_{:d}.pth'.format(sigma) # pre-trained network
    else: # grayscale video
        net = 'vnlnet_gray_{:d}.pth'.format(sigma) # pre-trained network
    
    cuda = not cpu and torch.cuda.is_available()
    training_args, model_state_dict = load_net(net, cuda)

    vmin = np.amin(video) # minimum value (range for normalization)
    vmax = np.amax(video) # maxium value (range for normalization)
    video = (video-vmin)/(vmax-vmin) # range normalized to [0,1]
    vdenoised, nntotaltime = test(training_args, model_state_dict, video, '', -1, cuda, False)

    return vdenoised*(vmax-vmin)+vmin
...
if __name__ == "__main__":
    ...
    parser_dir.add_argument('--ref', type=str, default="", help='Path to reference directory')
    ...
    start_time = time.time()
    video_denoised, nntotaltime = test(training_args, model_state_dict, video, foutnames, only_frame, cuda, args.add_noise)
    vnlnet_time = time.time() - start_time
    print('VNLnet denoising takes {:3.3f} seconds (non-local search {:3.3f} sec, {:2.1%}).'.format(vnlnet_time, nntotaltime, nntotaltime/vnlnet_time))
    
    if args.ref: # calculate quality metrics (PSNR and SSIM) with the reference video
        psnr.print_psnr(args.ref, args.output)
        ssim.print_ssim(args.ref, args.output)
