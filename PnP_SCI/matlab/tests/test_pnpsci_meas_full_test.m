% function test_pnpsci_meas_full_test(dataname)
...
% datasetdir = '../dataset/simdata/benchmark/data/original_benchmark'; % benchmark simulation dataset [original]
datasetdir = '../dataset/simdata/benchmark/data/combine_binary_mask_256_10f/bm_256_10f'; 
resultdir  = '../results';                   % results

test_algo_flag = 3;		% choose algorithms: 0-all, 1-gap-denoiser, 2-gap-denoisers 3-admm-denoiser
saving_data_flag = 0;	% save the recon result
% tv_init_flag = 0;		% use init input

% [1] load dataset
% if nargin<1
%     dataname = 'kobe'; % [default] data name
	dataname = 'data_kobe'; % [default] data name
% end
...
%% [2]  GAP-denoiser
if test_algo_flag==0 || test_algo_flag==1
	%% [2.1] GAP-TV
...
%% [3]  GAP-denoisers
if test_algo_flag==0 || test_algo_flag==2
	%% [3.1] GAP-WNNM-TV
	para.denoiser = 'wnnm'; % WNNM denoising
	  para.wnnm_int = true; % enable GAP-WNNM integrated
		para.blockmatch_period = 20; % period of block matching
...
%% [2]  ADMM-denoiser
if test_algo_flag==0 || test_algo_flag==3
%% [4.1] ADMM-TV
	para.denoiser = 'tv'; % TV denoising
	  para.maxiter  = 100; % maximum iteration
	  para.tvweight = 0.07*255/MAXB; % weight for TV denoising
	  para.tviter   = 5; % number of iteration for TV denoising
	  para.projmeth = 'admm'; % projection method

	[vadmmtv,psnr_admmtv,ssim_admmtv,tadmmtv,psnrall_admm_tv] = ...
		admmdenoise_cacti(mask,meas,orig,[],para);

	fprintf('admm-%s mean PSNR %2.2f dB, mean SSIM %.4f, total time % 4.1f s.\n',...
		upper(para.denoiser),mean(psnr_admmtv),mean(ssim_admmtv),tadmmtv);
...
% 	  para.projmeth = 'admm'; % projection method
% 
% 	[vadmmvbm4d,psnr_admmvbm4d,ssim_admmvbm4d,tadmmvbm4d,psnrall_admm_vbm4d] ...
% 	= admmdenoise_cacti(mask,meas,orig,[],para);
% 
% 	fprintf('admm-%s mean PSNR %2.2f dB, mean SSIM %.4f, total time % 4.1f s.\n',...
% 		upper(para.denoiser),mean(psnr_admmvbm4d),mean(ssim_admmvbm4d),tadmmvbm4d);
	...
	[vadmmffdnet,psnr_admmffdnet,ssim_admmffdnet,tadmmffdnet,psnrall_admm_ffdnet] = ...
		admmdenoise_cacti(mask,meas,orig,[],para);

	fprintf('admm-%s mean PSNR %2.2f dB, mean SSIM %.4f, total time % 4.1f s.\n',...
		upper(para.denoiser),mean(psnr_admmffdnet),mean(ssim_admmffdnet),tadmmffdnet);

	%% [4.4] admm-WNNM
% 	para.projmeth = 'admm'; % projection method
% 	para.denoiser = 'wnnm'; % WNNM denoising
% 	
% 	  para.wnnm_int = true; % enable admm-WNNM integrated
% 		para.blockmatch_period = 20; % period of block matching
% 	  para.sigma   = [100 50 25 12  6]/MAXB; % noise deviation (to be estimated and adapted)
% 	  para.vrange  = 1; % value range
% 	  para.maxiter = [ 60 60 60 60 60];
...
% 	  end
% 
% 	[vadmmwnnm,psnr_admmwnnm,ssim_admmwnnm,tadmmwnnm,psnrall_admm_wnnm] = ...
% 		admmdenoise_cacti(mask,meas,orig,[],para);
% 
% 	delete(poolobj); % delete pool object
% 
% 	fprintf('admm-%s mean PSNR %2.2f dB, mean SSIM %.4f, total time % 4.1f s.\n',...
% 		upper(para.denoiser),mean(psnr_admmwnnm),mean(ssim_admmwnnm),tadmmwnnm);
end


%% [5] save as the result .mat file and run the demonstration code
if saving_data_flag == 1
	%       demo_gapdenoise.
