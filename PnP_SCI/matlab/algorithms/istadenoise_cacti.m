function [v_,psnr_,ssim_,t_,psnrall] = istadenoise_cacti(mask,meas,orig,v0,para)
%ISTADENOISE_CACTI ISTA-Denoise frame for recontruction of  CACTI high-speed 
%imaging.
%   See also ISTADENOISE.
...
for kf = 1:nframe
    fprintf('ISTA-%s Reconstruction frame-block %d of %d ...\n',...
...
%         end
    elseif strcmp(para.denoiser,'wnnm') && isfield(para,'wnnm_int_fwise') && para.wnnm_int_fwise % GAP-WNNM integrated (with frame-wise denoising)
		error('NotImplemented')
		%         if isfield(para,'flag_iqa') && ~para.flag_iqa % ImQualAss disabled
...
%         end
        ...
        if isfield(para,'flag_iqa') && ~para.flag_iqa % ImQualAss disabled
            v = istadenoise(y,para);
        else
            [v,psnrall(kf,:)] = istadenoise(y,para);
        end
    end
