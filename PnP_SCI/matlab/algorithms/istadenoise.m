function [v, psnrall] = istadenoise( y , opt )
%ISTADENOISE Iterative Shrinkage Threshold Algorithm (ISTA)-based denoising
%framework for compressive sensing reconstruction.
%   v=ISTADENOISE(y,opt) returns the reconstruction result v of the
%   measurements with CASSI or CACTI coding, where y is the measurement
%   matrix, opt is the parameters for the ISTA-Denoise algorithm, typically
...
%     update Jul 13, 2018.
% 
%   See also ISTADENOISE_CACTI.
%	Created: Zhihong Zhang, 20200810
...
if isfield(opt,'gradF'), gradF = @(z) opt.gradF(z); end
...
for isig = 1:length(maxiter) % extension for a series of noise levels
    ...
    for iter = 1:maxiter(isig)
        % [1.1] update orig^
		v = v - lambda*gradF(v); % gradient decent
		alpha = 0.3;
		v = sign(v).*max(abs(v)-lambda*alpha, 0) ; % shirinkage threshold
        ...
        if flag_iqa && isfield(opt,'orig') && (~isempty(opt.orig))
            ...
            if (mod(k,5)==0) 
                fprintf('  ISTA-%s iteration % 4d, sigma % 3d, PSNR %2.2f dB.\n',...
