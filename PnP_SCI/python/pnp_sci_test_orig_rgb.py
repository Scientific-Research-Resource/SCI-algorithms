# =====
# Test PnP_SCI on RGB images (dealing with each channel separately)
# TODO: implement new recon functions for RGB image.
# =====
...
from utils import (A_, At_, show_n_save_res,save_rgb_img)
...
orig_dir = './dataset/orig/rgb'
mask_dir = './dataset/mask' # mask dataset
...
show_res_flag = 0           # NotImp: show results 
save_res_flag = 1          # save results
save_param_flag =  0        # NotImp: save params
save_resImg_flag = 1          # save recon resuls to images
# warning: choose algorithms (chose only one): 
# 'gaptv', 'admmtv', 'gapffdnet', 'admmffdnet', 
# 'gapfastdvdnet', 'admmfastdvdnet', 'gaptv+ffdnet', 'gaptv+fastdvdnet'
test_algo_flag = ['gapfastdvdnet']
...
if get_matfile_version(_open_file(origpath, appendmat=True)[0])[0] < 2: # MATLAB .mat v7.2 or lower versions
    ...
    with h5py.File(origpath, 'r') as origfile: # for '-v7.3' .mat file (MATLAB)
        orig = np.array(origfile['orig'])
        orig = np.float32(orig).transpose((3,2,1,0))
    with h5py.File(maskpath, 'r') as maskfile: # for '-v7.3' .mat file (MATLAB)
        # print(list(file.keys()))
        mask = np.array(maskfile['mask'])
        mask = np.float32(mask).transpose((3,2,1,0))

#  calc meas
# print(orig.shape, mask.shape)
assert np.ndim(orig)==4, 'Not RGB images'
nmask = mask.shape[3]
norig = orig.shape[3]
meas = np.zeros([orig.shape[0], orig.shape[1], 3, norig//nmask])
for i in range(norig//nmask):
    tmp_orig = orig[:,:,:,i*nmask:(i+1)*nmask]
    # print(tmp_orig.shape, mask.shape, meas.shape)
    meas[:,:,:,i] = np.sum(tmp_orig*mask, 3)

# zzh: expand dim for a single 'meas'
if meas.ndim<4:
    meas = np.expand_dims(meas,3)
    # print(meas.shape)
...
gaussian_noise = np.random.randn(*meas.shape)*gaussian_noise_level
...
#%% reconstruction each channel separately
mask_s = mask
meas_s = meas
orig_s = orig
vrecon = np.zeros([*mask_s.shape[0:3], nmask*nframe])
trecon,psnr_recon,ssim_recon = 0, [], []

for ch in range(3):
    print('===> processing channel {}/{}...'.format(1+ch,3))
    mask = mask_s[:,:,ch,:]
    meas = meas_s[:,:,ch,:]
    orig = orig_s[:,:,ch,:]
    
    # print(mask.shape, meas.shape, orig.shape, vrecon[:,:,ch,:].shape)
    ...
    if ('all' in test_algo_flag) or ('gaptv' in test_algo_flag):
        ...
        # vgaptv,tgaptv,psnr_gaptv,ssim_gaptv,psnrall_gaptv
        vgaptv,tgaptv,psnr_gaptv,ssim_gaptv,_ = admmdenoise_cacti(meas, mask, A, At,
        ...
        vrecon[:,:,ch,:] = vgaptv
        trecon += tgaptv
        psnr_recon.extend(psnr_gaptv)
        ssim_recon.extend(ssim_gaptv)
    ...
    if ('all' in test_algo_flag) or ('admmtv' in test_algo_flag):
        ...
        vrecon[:,:,ch,:] = vadmmtv
        trecon += tadmmtv
        psnr_recon.extend(psnr_admmtv)
        ssim_recon.extend(ssim_admmtv)
    ...
    if ('all' in test_algo_flag) or ('gapffdnet' in test_algo_flag):
        ...
        vrecon[:,:,ch,:] = vgapffdnet
        trecon += tgapffdnet
        psnr_recon.extend(psnr_gapffdnet)
        ssim_recon.extend(ssim_gapffdnet)
    ...
    if ('all' in test_algo_flag) or ('admmffdnet' in test_algo_flag):
        ...
        vadmmffdnet,tadmmffdnet,psnr_admmffdnet,ssim_admmffdnet,psnrall_admmffdnet = admmdenoise_cacti(meas, mask, A, At,
                                                ...
                                                sigma=sigma)
        vrecon[:,:,ch,:] = vadmmffdnet
        trecon += tadmmffdnet
        psnr_recon.extend(psnr_admmffdnet)
        ssim_recon.extend(ssim_admmffdnet)
    # %%
    ...
    if ('all' in test_algo_flag) or ('gapfastdvdnet' in test_algo_flag):
        ...
        vgapfastdvdnet,tgapfastdvdnet,psnr_gapfastdvdnet,ssim_gapfastdvdnet,psnrall_gapfastdvdnet = admmdenoise_cacti(meas, mask, A, At,
                                                ...
                                                denoiser=denoiser, model=model, 
                                                iter_max=iter_max, sigma=sigma)
        vrecon[:,:,ch,:] = vgapfastdvdnet
        trecon += tgapfastdvdnet
        psnr_recon.extend(psnr_gapfastdvdnet)
        ssim_recon.extend(ssim_gapfastdvdnet)  
    ...
    if ('all' in test_algo_flag) or ('admmfastdvdnet' in test_algo_flag):
        ...
        vrecon[:,:,ch,:] = vadmmfastdvdnet
        trecon += tadmmfastdvdnet
        psnr_recon.extend(psnr_admmfastdvdnet)
        ssim_recon.extend(ssim_admmfastdvdnet)  
    # %%
    ...
    if ('all' in test_algo_flag) or ('gaptv+ffdnet' in test_algo_flag):
        ...
        vrecon[:,:,ch,:] = vgaptvffdnet
        trecon += tgaptvffdnet
        psnr_recon.extend(psnr_gaptvffdnet)
        ssim_recon.extend(ssim_gaptvffdnet)  
    ...
    if ('all' in test_algo_flag) or ('admmtv+ffdnet' in test_algo_flag):
        ...
        vrecon[:,:,ch,:] = vadmmtvffdnet
        trecon += tadmmtvffdnet
        psnr_recon.extend(psnr_admmtvffdnet)
        ssim_recon.extend(ssim_admmtvffdnet)  
    ...
    if ('all' in test_algo_flag) or ('gaptv+fastdvdnet' in test_algo_flag):
        ...
        vgaptvfastdvdnet,tgaptvfastdvdnet,psnr_gaptvfastdvdnet,ssim_gaptvfastdvdnet,psnrall_gaptvfastdvdnet = joint_admmdenoise_cacti(meas, mask, A, At,
                                                ...
                                                tv_weight=tv_weight, tv_iter_max=tv_iter_max, 
                                                model=model, sigma1=sigma1, sigma2=sigma2, tvm=tvm)
        vrecon[:,:,ch,:] = vgaptvfastdvdnet
        trecon += tgaptvfastdvdnet
        psnr_recon.extend(psnr_gaptvfastdvdnet)
        ssim_recon.extend(ssim_gaptvfastdvdnet)  
    ...
    if ('all' in test_algo_flag) or ('admmtv+fastdvdnet' in test_algo_flag):
        ...
        vrecon[:,:,ch,:] = vadmmtvfastdvdnet
        trecon += tadmmtvfastdvdnet
        psnr_recon.extend(psnr_admmtvfastdvdnet)
        ssim_recon.extend(ssim_admmtvfastdvdnet)  
        
#%% print recon info and save results       
print('-'*20+'\n{}-{} PSNR {:2.3f} dB, SSIM {:.4f}, running time {:.1f} seconds.\n'.format(
    projmeth.upper(), denoiser.upper(), mean(psnr_recon), mean(ssim_recon), trecon)+'-'*20)
show_n_save_res(vrecon,trecon,psnr_recon,ssim_recon,[], orig, nmask, resultsdir, 
                    projmeth+denoiser+'_'+orig_name, iframe=iframe,nframe=nframe, MAXB=MAXB,show_res_flag=show_res_flag, save_res_flag=save_res_flag)
if save_resImg_flag:
    save_rgb_img(vrecon, resultsdir+'/savedimg/'+projmeth+denoiser+'_'+orig_name, rescale_ch=True)
